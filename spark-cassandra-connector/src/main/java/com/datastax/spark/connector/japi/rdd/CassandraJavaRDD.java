...
public class CassandraJavaRDD<R> extends JavaRDD<R> {
    ...
    public CassandraJavaRDD<R> select(String... columnNames) {
        ...
        CassandraRDD<R> newRDD = (CassandraRDD<R>) rdd().select(columnRefs);
        return wrap(newRDD);
    }
    ...
    public CassandraJavaRDD<R> withAscOrder() {
        CassandraRDD<R> newRDD = (CassandraRDD<R>) rdd().withAscOrder();
        return wrap(newRDD);
    }
    ...
    public CassandraJavaRDD<R> withDescOrder() {
        CassandraRDD<R> newRDD = (CassandraRDD<R>) rdd().withDescOrder();
        return wrap(newRDD);
    }
    ...
    public CassandraJavaRDD<R> limit(Long rowsNumber) {
        CassandraRDD<R> newRDD = (CassandraRDD<R>) rdd().limit(rowsNumber);
        return wrap(newRDD);
    }
